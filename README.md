...
(`common/web/SpaController` serves `<tier>/web/app/index.html`, injecting `window.__TIZBIZ__`).

| Tier | Domain | SPA (`apps/*`) | Role |
|---|---|---|---|
| `frontend/` | `tizbiz.uz` | `corporate` | Corporate seller / marketing page |
| `backend/` | `admin.tizbiz.uz` | `admin` | Business admin dashboard |
| `tenant/` | `{slug}.tizbiz.uz` | `booking` | Per-business consumer booking site |
| `api/` | `api.tizbiz.uz` | — | REST API (JSON, JWT, RBAC-by-role, CORS) |
| `console/` | — | — | Migrations & CLI (`migrate`, `seed`, `reminder`) |
| `common/` | — | — | Shared models, JWT, tenant scoping, REST base, envelope |

The tenant tier reads the business slug from the `Host` header (wildcard `*.tizbiz.uz`) and
...
export DB_DSN='mysql:host=127.0.0.1;dbname=tizbiz' DB_USER='root' DB_PASSWORD='' JWT_SECRET='change-me'
...
# dev with hot reload (calls the API at window.__TIZBIZ__.apiBase, default http://127.0.0.1:8899):
corepack pnpm --filter @tizbiz/admin dev
```
